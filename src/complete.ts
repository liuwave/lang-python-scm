...
const gatherCompletions: {
  ...
  ParamList(node, def) {
    for (
      let prev = null as never as SyntaxNode, child = node.node.firstChild;
      child;
...
export const globalCompletion = (completions: Completion[]=[]) => {
  return ifNotIn(
    dontComplete,
    completeFromList(completions.concat(globals.concat(snippets)))
  );
};
